...
export default async function LoginPage({ searchParams }: { searchParams: { error?: string } }) {
  // Wrap in try/catch: if AUTH_SECRET is missing or session cookie is corrupt
  // we must still show the login form, never a 500.
  try {
    const user = await getCurrentUser();
    if (user) redirect(user.role === Role.ADMIN ? "/admin" : "/team");
  } catch {
    // Session read failed (e.g. misconfigured AUTH_SECRET) — show login form anyway
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-canvas px-4 py-10">
      <div className="w-full max-w-md rounded-lg border border-line bg-white p-6 shadow-panel">
        ...
        <form action="/api/auth/login" method="post" className="mt-6 space-y-4">
          ...
          <button className="focus-ring h-10 w-full rounded-md bg-brand text-sm font-semibold text-white hover:bg-[#12564C]">
            Sign in
          </button>
        </form>
      </div>
